...
namespace Drupal\govcore\Commands;
...
/**
 * Hooks into Drush to assist with uninstalling GovCore.
 *
 * @internal
...
final class Uninstaller extends DrushCommands implements SiteAliasManagerAwareInterface {
  ...
  /**
   * Defines dynamic options if uninstalling GovCore.
   *
   ...
   *
   * @return string|null
   *   If an uninstall of GovCore or Headless GovCore is being attempted,
  ...
  /**
   * Validates the pm:uninstall command if GovCore is being uninstalled.
   *
   ...
   *   Thrown if:
   *   - The user is trying to uninstall GovCore (or Headless GovCore) at
   *     the same time as other extension(s).
   *   - Any installed extensions are physically located inside the GovCore
   *     profile directory.
   *   - Any profiles (installed or not) are using GovCore or Headless
   *     GovCore as their immediate parent, and the user declines to modify
   *     them automatically.
   */
  public function validate(CommandData $data) : void {
    ...
    if ($profile) {
      ...
      // Ensure that there are no installed modules or themes in the GovCore
      // profile directory.
      $extensions = $this->getExtensionsInProfileDirectory();
      if ($extensions) {
        $error = sprintf('The following modules and/or themes are located inside the GovCore profile directory. They must be moved elsewhere before GovCore can be uninstalled: %s', implode(', ', $extensions));
      ...
      if ($children) {
        $warning = sprintf('The following install profiles use %s as a base profile. They must stand alone, or use a different base profile, before GovCore can be uninstalled: %s', $info['name'], implode(', ', $children));
  ...
  /**
   * Performs required actions before GovCore is uninstalled.
   *
   * @hook pre-command pm:uninstall
   */
  public function preCommand() : void {
    if ($this->getUninstall()) {
      if ($this->installProfile === 'govcore' || $this->installProfile === 'headless_govcore') {
        // The govcore_install module was created to prevent broken builds of
        // GovCore (created by drupal.org's legacy packaging system) from
        // being installed.
        $this->drush('pm:uninstall', ['govcore_install']);
  ...
  /**
   * Performs required actions after GovCore is uninstalled.
   *
  ...
  /**
   * Returns installed extensions in the GovCore profile directory.
   *
   * @return string[]
   *   The names of installed extensions in the GovCore profile directory.
   *   If there are any, GovCore cannot be uninstalled. There shouldn't
   *   normally be any -- GovCore doesn't ship with any runtime modules --
  ...
  private function getExtensionsInProfileDirectory() : array {
    ...
    $profile_path = $extensions['govcore']->getPath();
    unset($extensions['govcore']);
    // The govcore_install module is a special module that was created to
    // prevent installation of broken builds of GovCore created by the legacy
    // drupal.org packaging system. This utility uninstalls it along with the
    // profile.
    unset($extensions['govcore_install']);
  ...
  /**
   * Alters the project-level composer.json to uninstall GovCore.
   */
  private function alterProject() : void {
    ...
    $this->io()->listing([
      'Ensuring direct GovCore dependencies are required',
    ...
    // Read GovCore's composer.json, since we will need to merge in many
    ...
    assert($source['name'] === 'acquia/govcore');
    ...
    // If the project requires Headless GovCore, we need to switch it to
    // Headless GovCore 2 to ensure that the modules which are part of
    // Headless GovCore will continue to be present after uninstall.
    if (isset($data['require']['acquia/headless_govcore'])) {
      $data['require']['acquia/headless_govcore'] = 'dev-eol';
    }
  ...
  private function getRequirements(array $target, array $source) : array {
    $requirements = [];
    // GovCore requires composer/composer in order for this uninstaller to
    ...
    // dependencies defined by the source package (GovCore).
  ...
  private function getPaths(array $target, array $source) : array {
    ...
    // If the target package uses Headless GovCore, it should be treated as
    // a normal module.
    if (isset($target['require']['acquia/headless_govcore'])) {
      $path_map += [
        'acquia/headless_govcore' => $path_map['type:drupal-module'],
      ];
    }
  ...
  private function getPathMap(array $target, array $source) : array {
    ...
    // falling back to the source package (GovCore) in the unlikely event
   ...
   *   The target package's configuration.
   * @param array $source
   *   The source package (i.e., GovCore)'s configuration.
   *
   * @return string|null
