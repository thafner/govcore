...
namespace Drupal\govcore\Generators;
...
/**
 * Implements a GovCore sub-profile generator for Drush.
 *
 ...
 * profile, and GovCore modules to exclude.
 */
final class SubProfileGenerator extends BaseGenerator {
   ...
   * {@inheritdoc}
   */
  protected $name = 'govcore-subprofile';

  /**
   * {@inheritdoc}
   */
  protected $description = 'Generates a GovCore Subprofile.';
  ...
  protected function interact(InputInterface $input, OutputInterface $output) {
    ...
    if ($io->confirm('Do you want to exclude any components of GovCore?', FALSE)) {
      $filter = function ($name) {
        return strpos($name, 'govcore_') === 0;
      };
      ...
      $questions['exclude'] = new ChoiceQuestion('GovCore components to exclude (optional), entered as keys separated by commas (e.g. govcore_media, govcore_layout)', $modules);
