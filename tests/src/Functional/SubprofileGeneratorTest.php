...
use Drupal\govcore\Generators\SubProfileGenerator;
use Drupal\Tests\BrowserTestBase;
use Drush\TestTraits\DrushTestTrait;

/**
 * Tests the Drush command to generate a GovCore sub-profile.
 *
 * @covers \Drupal\govcore\Generators\SubProfileGenerator
 *
 * @group govcore
 */
class SubprofileGeneratorTest extends BrowserTestBase {
  ...
  public function provider() {
    return [
      ...
      'with exclusions' => [
        'answers' => [
          'exclude' => 'govcore_search, govcore_media_instagram',
        ],
      ],
      'inclusions and exclusions' => [
        'answers' => [
          'install' => ['consumers', 'book'],
          'exclude' => 'govcore_media_instagram',
        ],
      ],
      'default answers' => [
        'answers' => [],
  ...
  /**
   * Tests the govcore-subprofile generator for Drush.
   *
   * @param array $answers
   *   The answers to the generator's questions, keyed by the questions' machine
   *   names.
   *
   * @see \Drupal\govcore\Generators\SubProfileGenerator::interact()
   *
  ...
  public function test(array $answers = []) {
    ...
    // We need Drush to run commands defined by the GovCore profile, but we
    // don't want to actually install the GovCore profile into the test site
    // because it takes freaking forever. So, this hack is a quick way to expose
    // our generator to Drush.
    $this->config('core.extension')
      ->set('module.govcore', 0)
      ->set('profile', 'govcore')
      ->save();
    ...
    $this->drush('generate', ['govcore-subprofile'], $options, NULL, "$drupal_root/..", 0, NULL, [
      'SHELL_INTERACTIVE' => 1,
    ]);
    ...
    $this->assertSame('govcore', $info['base profile']);
